...
/// assemblies next to this test (the WebFlux packages, the sibling packages they integrate and the
...
public class DocsSnippetRosterTests
{
    ...
    private static readonly HashSet<string> KnownExternal = new(StringComparer.Ordinal)
    {
        ...
        "ReadAllAsync",                // System.Threading.Channels, in the architecture guide's pipeline sketch
        "ToFloats",                    // OpenAI SDK, in the tutorial's own embedding service
        "CompleteChatAsync",           // OpenAI SDK, in the tutorial's own completion service
        "CompleteChatStreamingAsync",  // OpenAI SDK, same
        "InsertAsync",                 // the reader's vector database in the tutorial's storage samples
        "UpsertAsync",                 // same
        "SearchAsync",                 // same
    };
    ...
    public void EveryNameADocumentUses_Exists_ExceptTheKnownDrift()
    {
        ...
        blocks.Should().BeGreaterThan(50, "the scan must find the documentation's C# blocks, or an empty finding list proves nothing");
    ...
    public void APhantomProperty_AndAPhantomType_AreReported()
    {
        ...
        findings.Should().BeEquivalentTo(["CrawlOptions.NoSuchOption", "CrawlOptions.Nested", "ImaginaryOptions (no such type in the library assemblies)"]);
    }
    ...
    public void APhantomRegistrationCall_IsReported_AndARealOneIsNot()
    {
        ...
        var calls = RegistrationCalls(
            "services.AddWebFlux().AddLogging().UseImaginaryThing();\n" +
        ...
        calls.Should().BeEquivalentTo(["AddWebFlux", "AddLogging", "UseImaginaryThing", "NoSuchProcessAsync", "ToUpperInvariant"]);
    ...
    /// <summary>
    /// The assemblies whose public method names count as "exists": the WebFlux packages, the sibling
    ...
    [
        "WebFlux*.dll", "Flux.*.dll", "FluxCurator*.dll", "Microsoft.Extensions.*.dll", "Microsoft.Playwright*.dll",
        "AngleSharp*.dll",
    ];
    ...
    private static string RepositoryRoot()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "WebFlux.slnx")))
            dir = dir.Parent;
        return dir?.FullName ?? throw new InvalidOperationException("WebFlux.slnx not found above the test output directory");
    }
}
