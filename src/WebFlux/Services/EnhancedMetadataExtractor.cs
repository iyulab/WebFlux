...
/// </summary>
public class EnhancedMetadataExtractor : IEnhancedMetadataExtractor
{
    ...
    /// </summary>
    public async Task<EnhancedWebMetadata> ExtractMetadataAsync(
        string htmlContent,
        string sourceUrl,
    ...
    /// </summary>
    private async Task<SchemaOrgMetadata> ExtractSchemaOrgMetadataAsync(HtmlDocument doc, CancellationToken cancellationToken)
    {
    ...
    private TechnicalMetadata ExtractTechnicalMetadata(HtmlDocument doc, string sourceUrl)
    {
        ...
        var isAmp = doc.DocumentNode.SelectSingleNode("//html[@amp]") != null || doc.DocumentNode.SelectSingleNode("//html[@⚡]") != null;
    ...
    /// 메타데이터 품질 점수 계산
    /// </summary>
    public double CalculateQualityScore(EnhancedWebMetadata metadata)
    {
    ...
    /// 메타데이터 완성도 평가
    /// </summary>
    public MetadataCompleteness EvaluateCompleteness(EnhancedWebMetadata metadata)
    {
        var basicScore = CalculateBasicMetadataScore(metadata.Basic);
        var ogScore = CalculateOpenGraphScore(metadata.OpenGraph);
