...
- [ ] `WebFluxServiceCollection` 확장 메서드 구현
- [ ] 서비스 등록 구성 클래스 구현
- [ ] 인터페이스별 라이프타임 관리 설정
- [ ] 구성 옵션 바인딩 구현
- [ ] 서비스 팩토리 패턴 구현

**완료 기준**: DI 컨테이너를 통한 모든 서비스 해결 가능, 구성 옵션 바인딩 검증

#### 2.2 Mock 서비스 구현 (테스트용 AI 서비스 구현체)
**목표**: 테스트 및 데모를 위한 가짜 구현체 제공
- [ ] `MockTextCompletionService` - 가짜 LLM 응답 생성
- [ ] `MockImageToTextService` - 가짜 이미지 설명 생성
- [ ] `MockEmbeddingService` - 가짜 벡터 임베딩 생성 (옵션)
- [ ] 응답 지연 시뮬레이션 기능
- [ ] 다양한 시나리오 테스트 데이터 제공
- [ ] 오류 상황 시뮬레이션 기능

**완료 기준**: Mock 서비스로 전체 파이프라인 동작 검증 완료
...
- [ ] `XmlContentExtractor` - XML 데이터 처리
- [ ] `TextContentExtractor` - 일반 텍스트 처리
...
#### 2.5 기본 청킹 전략 구현
**목표**: 기본적인 청킹 알고리즘 구현
- [ ] `FixedSizeChunkingStrategy` - 고정 크기 분할
- [ ] `ParagraphChunkingStrategy` - 문단 기반 분할
- [ ] `ChunkingStrategyFactory` - 전략 팩토리 구현
- [ ] 청크 품질 평가 메트릭 구현
- [ ] 청크 겹침(Overlap) 처리 로직
- [ ] 토큰 계산 유틸리티 구현

**완료 기준**: 기본 청킹 전략으로 품질 점수 75% 이상 달성

#### 2.6 파이프라인 오케스트레이션 구현
**목표**: 크롤링 → 추출 → 청킹 파이프라인 구현
- [ ] `WebContentProcessor` 메인 클래스 구현
- [ ] Threading.Channels 기반 파이프라인 구현
- [ ] 백프레셔 제어 메커니즘
- [ ] 병렬 처리 관리
- [ ] 오류 처리 및 복구 로직
- [ ] 파이프라인 상태 관리

**완료 기준**: End-to-End 파이프라인 동작 검증, 처리율 목표 달성
...
#### Phase 2 완료 기준
- [ ] DI 컨테이너를 통한 모든 서비스 해결 가능
- [ ] Mock 서비스로 전체 파이프라인 동작 검증
- [ ] 실제 웹사이트 크롤링 및 청킹 성공
- [ ] 기본 청킹 전략 성능 벤치마크 완료
